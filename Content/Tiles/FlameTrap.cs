...
{
    public class FlameTrap : ModTile
    {
        public override void SetStaticDefaults()
		{
            ...
            TileObjectData.newTile.HookPostPlaceMyPlayer = new PlacementHook(ModContent.GetInstance<TEFlameTrap>().Hook_AfterPlacement, -1, 0, false);
        ...
        public override void KillTile(int i, int j, ref bool fail, ref bool effectOnly, ref bool noItem)
        {
            if (!fail)
            {
                ModContent.GetInstance<TEFlameTrap>().Kill(i, j);
            }
        }
    ...
    public class TEFlameTrap : ModTileEntity
    {
        public override bool IsTileValidForEntity(int x, int y)
        {
            Tile tile = Main.tile[x, y];
            return tile.HasTile && tile.TileType == ModContent.TileType<FlameTrap>();
        }
        ...
        public override void Update()
        {
            if (Main.GameUpdateCount % 240 == 120)
            {
                ...
                Vector2 position = (Position.ToVector2() + Vector2.UnitX / 2 - Vector2.UnitY * 2.5f) * 16;
                Projectile proj = Projectile.NewProjectileDirect(Projectile.GetSource_None(), position, Vector2.Zero, ModContent.ProjectileType<FlamePillar>(), 50, 0);
